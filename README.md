...
- **评论随消息自动携带（Codex 式）**：行内评论以紧凑 **chip 条常驻**在**输入框上方**
  （与输入卡同面料、视觉融合，不悬浮、不跳动——每条 chip 显示 `路径:行号` +
  摘要，**点击 chip → 在评审面板中打开对应变更块**，超过 4 条折叠为 +N）；
  **顶栏本身就是发送控制**——无需单独的发送按钮，点击顶栏立即把评审
  包发出去，或照常在输入框发送下一条消息时自动附带——完整的评审包（每条评论 +
  对应 diff hunk + AI 评审结论 verdict 与发现，如有）以 steering 注入——代理
  下一步即处理，不再出现「排队信息」条；
  评审发现与 PR 评论仍通过面板内「发送」弹层聚合；
- **会话里的评审卡片**：发送出去的评审包在对话中渲染为一张**可点击卡片**（类似
  Codex 的评审摘要）——按文件分组列出每条评论（`路径:行号` + 正文）与 AI 评审
  结论，**点击任意评论或文件 → 在评审面板中定位到对应变更块/文件**；发送过的
  评论不会重复发送（持久化于每个工作区）；
...
| **评论浮层** | 输入框上方以 chip 条常驻显示待发送评论（点击 chip 跳转对应变更，超过 4 条折叠 +N），点击顶栏或随消息发送时自动附带 |

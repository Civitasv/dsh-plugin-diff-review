...
- **行内评论 chip 条（Codex 式）**：行内评论以紧凑 **chip 条常驻**在**输入框上方**
  （与输入卡同面料、视觉融合，不悬浮、不跳动——每条 chip 显示 `路径:行号` +
  摘要，**点击 chip → 在评审面板中打开对应变更块**，超过 4 条折叠为 +N）；
  **顶栏本身就是发送控制**——无需单独的发送按钮，**只有显式点击顶栏才会发送**：
  完整的评审包（每条评论 + 对应 diff hunk + AI 评审结论 verdict 与发现，如有）
  以 steering 注入——代理下一步即处理，不再出现「排队信息」条；发送普通消息
  **不会**自动附带评论；
...
| **评论浮层** | 输入框上方以 chip 条常驻显示待发送评论（点击 chip 跳转对应变更，超过 4 条折叠 +N），仅显式点击顶栏发送 |

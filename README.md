...
| **拖拽调整宽高** | 拖动面板右边缘 / 下边缘 / 右下角可动态调整面板尺寸，位置与大小会记住（最小 640×400） |
...
在 **设置 → 通用 → Diff 审查** 一行中可修改 diff 的字体与字号：

- **字体**：等宽（默认，跟随 DSH 主题）/ 系统字体 / Consolas / JetBrains Mono /
  Fira Code / Source Code Pro；
- **字号**：11–18px（默认 12px）。

设置即时生效并持久化，同时作用于单栏与双栏视图（含行号、文件列表）。
